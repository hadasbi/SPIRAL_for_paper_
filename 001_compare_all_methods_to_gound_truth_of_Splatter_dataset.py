...
def ARI_Hullermeier2012(gene_list, clustering1, clustering2):   
    ...
    d = 0
    # add score for every gene pair
    for ig1, g1 in enumerate(gene_list):
        if (ig1+1)%100==0:
            print(ig1, '/', ngenes)
        for g2 in gene_list[ig1+1:]:
            E1 = 1 - np.linalg.norm(clustering1.loc[g1,:]-clustering1.loc[g2,:], ord=1)/nclus1
            E2 = 1 - np.linalg.norm(clustering2.loc[g1,:]-clustering2.loc[g2,:], ord=1)/nclus2
            d += np.abs(E1-E2)
    d = d/(ngenes*(ngenes-1)/2)
    ARI = 1 - d
    return ARI
    '''
...
'''
def our_rand_index(gene_list, cluster_table1, cluster_table2):
    cluster_dict1 = dict()
    for clus in cluster_table1.index:
        # read cluster gene list
        cluster_dict1[clus] = cluster_table1.loc[clus1, 'genes'].replace("['", "").replace("']", "").split("', '")
        
    cluster_dict2 = dict()
    for clus in cluster_table2.index:
        # read cluster gene list
        cluster_dict2[clus] = cluster_table2.loc[clus, 'genes'].replace("['", "").replace("']", "").split("', '")
    
    s = 0
    for ig1, g1 in enumerate(gene_list):
        for g2 in gene_list[ig1+1:]:
            r1 = len([clus for clus in cluster_dict1.keys() if g1 in cluster_dict1[clus] and g2 in cluster_dict1[clus]])
            r2 = len([clus for clus in cluster_dict2.keys() if g1 in cluster_dict2[clus] and g2 in cluster_dict2[clus]])
            s += abs(r1-r2)
            
    return s/(len(gene_list)*(len(gene_list) - 1)/2)
'''
